...
In order to evaluate the model trends, the region time series have been computed with the models output by colocating the data to the available observations in space and time. The trends of both observations and models are shown in Figure \ref{fig:bars} with their associated uncertainties. The models performances for the reproduction the observed trends vary from one parameter to the other.

\begin{itemize}
    \item AOD: the trends are pretty well captured by all the models over all the regions except in Asia, where all the models show increasing trends while a small and not significant decrease is found in the observations. However, the associated uncertainties are, in this case, usually larger than the trends values. Some differences can be noticed in between the 3 categories of models:
    \begin{itemize}
        \item CAMS Rean: 
        \item AP3: 
        \item CMIP6: 
