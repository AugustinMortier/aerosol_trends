...
\begin{itemize}
 ...
 \item Absorption coefficients (\unit{Mm^{-1}}): from aethalometer - validity range: [] - Validity range: []
\end{itemize}
...
\begin{equation}
 Rep(p, r) = \frac{f\left(\left| s_{Exp_{t}(p, r)}-s_{Ref_{t}(p, r)} \right|\right) + f\left\left| s_{Exp_{s}(p, r)}-s_{Ref_{s}(p, r)}\right|\right) }{2} \right)
\end{equation}
